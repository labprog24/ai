...
class State:
    ...
    def __eq__(self, other):
        ...
        return (
            self.left_missionaries, self.left_cannibals, self.boat,
            self.right_missionaries, self.right_cannibals
        ) == (
            other.left_missionaries, other.left_cannibals,
            other.boat, other.right_missionaries,
            other.right_cannibals
        )
...
def generate_successors(current_state):
    ...
    for action in actions:
        if current_state.boat == 0:
            ...
            new_boat = 0
        if (
            0 <= new_left_missionaries <= 3 and
            0 <= new_left_cannibals <= 3 and
            0 <= new_right_missionaries <= 3 and
            0 <= new_right_cannibals <= 3
        ):
            new_state = State(
                new_left_missionaries, new_left_cannibals, new_boat,
                new_right_missionaries, new_right_cannibals
            )
